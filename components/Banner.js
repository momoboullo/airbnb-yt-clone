import Image from "next/image";

function Banner() {
  return (
    <div className="relative h-[300px] sm:h-[400px] lg:h-[500px] xl:h-[600px] 2xl:[650px]">
      <Image
        src="https://hbr.org/resources/images/article_assets/2019/04/Apr19_17_970167284.jpg"
        layout="fill"
        objectFit="cover"
      />
      <div className="relative top-1/2 w-full text-center">
        <p className="text-sm sm:text-lg">Not sure where to go? Perfect.</p>
        <button className="text-purple-500 bg-white px-10 py-4 shadow-md rounded-full font-bold my-3 hover:shadow-xl active:scale-90 duration-150">
          I'm flexible
        </button>
      </div>
    </div>
  );
}

export default Banner;
